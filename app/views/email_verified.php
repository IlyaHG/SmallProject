...
if (!$auth->isLoggedIn()) {
    ...
    header("Location: /public/page_login");
    exit;
}


$current_user = $auth->getUserId();
$trimmedUserGET =trim($_GET['route'],'status/id=');
    ...
    <main id="js-page-content" role="main" class="page-content mt-3">
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-sun'></i> Ваш Email Подтвержден
            </h1>
        </div>
        <form action="/users" >
            <div class="row">
                <div class="col-xl-6">
                    <div class="col-md-4">
                        <button class="btn btn-warning" >Перейти к пользователям</button>
                    </div>
                </div>
            </div>
        </form>
